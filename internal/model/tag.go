...
// Insert `Tag` value into the table.
...
func (t Tag) TagExists(db *gorm.DB) (*Tag, bool) {
	var target Tag

	err := db.Where("name = ? AND created_by = ?", t.Name, t.CreatedBy).First(&target).Error
	if err != nil {
		return nil, false
	}
...
func (t Tag) QueryTag(db *gorm.DB) (*Tag, error) {
	err := db.Where("name = ? AND created_by = ?", t.Name, t.CreatedBy).First(&t).Error
	if err != nil {
		return nil, err
	}
